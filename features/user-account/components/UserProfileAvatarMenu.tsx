...
const UserProfileAvatarMenu = () => {
  ...
  return (
    <HoverCard>
      ...
      <HoverCardContent
        align="start"
        className="backdrop-blur bg-primary-300/40  text-primary-900 text-sm"
      >
            ...
            ({ url, label }, index) => {
              return (
                <Link
                  href={`${url}`}
                  key={index}
                  className=" hover:backdrop-blur hover:bg-priamry-600/10 px-2 py-2 rounded-md block transition-all ease-in-out duration-300 w-full "
                >
                  {label}
                </Link>
              );
